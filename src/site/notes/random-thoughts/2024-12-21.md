---
{"dg-publish":true,"dg-permalink":"random-thoughts/2024-12-21","permalink":"/random-thoughts/2024-12-21/","title":"Kafka 架構深入解析：從元件互動到實務實作指南","tags":["kafka","architecture","distributed-systems","message-queue","backend"]}
---
...
- The flow of how consumer consumes messages from Kafka:
  ...
  ```
  The behavior of Fetcher in Kafka Consumer:
  Class Diagram:
  ```mermaid
  classDiagram
    class Fetcher {
  ...
  the difference between Node and Broker in Kafka:
  ...
  sequenceDiagram
      ...
      Client->>Metadata: currentLeader(partition)
      activate Metadata
      Metadata->>Broker: getLeaderFor(partition)
      Broker-->>Metadata: brokerId
      Metadata->>Node: getNode(brokerId)
      Node-->>Metadata: node
      Metadata-->>Client: node
      deactivate Metadata
      
      Client->>Node: connect()
      Note over Client,Node: Network operations<br/>use Node object
      
      Note over Broker: Broker contains full<br/>server-side state
  ```
  ...
  2. Broker is used for:
  - Server-side state
  - Cluster metadata
  - Role management
  - Configuration

  3. In Consumer Context:
